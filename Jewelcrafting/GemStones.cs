...
public static class GemStones
{
    ...
    public static readonly List<string> socketableGemStoneNames = new();
    ...
    private class LimitTabToGemCutterTable
    {
        ...
        {
            AddSocketAddingTab.tab.gameObject.SetActive(Player.m_localPlayer.m_currentStation && Player.m_localPlayer.m_currentStation.name.StartsWith("op_transmution_table", StringComparison.Ordinal) && Jewelcrafting.socketingTab.Value == Jewelcrafting.Toggle.On);
    ...
    {
        public static readonly GameObject[] socketIcons = new GameObject[Jewelcrafting.maxInternalNumberOfSockets];
        public static Button socketingButton = null!;
        public static GameObject containerCloseContainer = null!;
        ...
        private static void Postfix(InventoryGui __instance)
        {
            ...
            for (int i = 0; i < Jewelcrafting.maxInternalNumberOfSockets; ++i)
            {
            ...
            Button.ButtonClickedEvent openButtonClick = new();
            openButtonClick.AddListener(() => { OpenFakeSocketsContainer.Open(__instance, __instance.m_selectedRecipe.ItemData); });
            socketingButton.onClick = openButtonClick;
            ...
            Button.ButtonClickedEvent closeButtonClick = new();
            closeButtonClick.AddListener(() => { CloseFakeSocketsContainer.Prefix(__instance); });
            containerCloseButton.GetComponent<Button>().onClick = closeButtonClick;
        }
    }
    ...
    private class AddSocketToItem
    {
        private static bool Prefix(InventoryGui __instance)
        {
            ...
            if (!Player.m_localPlayer.m_noPlacementCost && Random.value > successChance)
            {
                ...
                if (socketedItem.Data()["Blessed Item"] is not null)
                {
                    socketedItem.Data().Remove("Blessed Item");
                    socketedItem.Data().Save();
                    Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$jc_blessed_triggered");
                }
                else if (Jewelcrafting.socketCost.Value != Jewelcrafting.SocketCost.CostsItems)
                {
                    if (socketedItem.Data().Get<Sockets>() is { } sockets && (Jewelcrafting.gemReturnLockedGems.Value == Jewelcrafting.Toggle.On || socketedItem.Data()["SocketsLock"] is null))
                    {
                        ...
                        foreach (ItemDrop itemDrop in itemInventory.GetAllItems().Select(gem => ItemDrop.DropItem(gem, 1, playerPosition.position + playerPosition.forward + playerPosition.up, playerPosition.rotation)))
                        {
                            ...
                            itemDrop.GetComponent<Rigidbody>().linearVelocity = (playerPosition.forward + Vector3.up) * 5f;
                ...
                {
                    Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$jc_socket_adding_fail_costsonly");
                }
    ...
    public class DisplaySocketTooltip
    {
        ...
        private static void Postfix(InventoryGrid __instance, ItemDrop.ItemData item, UITooltip tooltip)
        {
            ...
            if (item.Data().Get<ItemContainer>() is not null || item.Data()["ProphecySeed"] is not null)
            {
    ...
    private class UpdateSocketTooltip
    {
        ...
        {
            if (UITooltip.m_tooltip?.transform.Find("Bkg (1)") is { } transform && global::Utils.GetPrefabName(UITooltip.m_tooltip) == GemStoneSetup.SocketTooltip.name && DisplaySocketTooltip.tooltipItem.TryGetValue(__instance, out Tuple<InventoryGrid?, ItemInfo> itemInfo))
            {
                FillItemTooltip(itemInfo.Item2.ItemData, transform, itemInfo.Item1 == InventoryGui.instance?.m_playerGrid);
            }
        }
    }

    public static void FillItemTooltip(ItemDrop.ItemData? item, Transform root, bool showInteract)
    {
        if (item is null)
        {
            return;
        ...
        string tooltipText = "";
        ItemContainer? container = item.Data().Get<ItemContainer>();
        string? prophecySeed = item.Data()["ProphecySeed"];
        ReplaceTooltipText.prophecyKeyDown = ZInput.GetButton("Run");
        ...
        if (container is not null)
        {
            if (showInteract)
            {
                ...
                {
                    tooltipText = Localization.instance.Localize("$jc_press_frame_interact", Localization.instance.Localize("<color=yellow><b>$KEY_Use</b></color>"));
                }
                else if (Jewelcrafting.inventoryInteractBehaviour.Value != Jewelcrafting.InteractBehaviour.Enabled && container is ItemBag)
                {
                    tooltipText = Localization.instance.Localize("$jc_press_gem_bag_interact", Localization.instance.Localize("<color=yellow><b>$KEY_Use</b></color>"));
                }
                ...
                {
                    tooltipText = Localization.instance.Localize(container is Box { progress: >= 100 } ? "$jc_gembox_interact_finished" : "$jc_press_interact", Localization.instance.Localize("<color=yellow><b>$KEY_Use</b></color>"));
                }
                else
                {
                    tooltipText = Localization.instance.Localize("$jc_table_required");
                }
            ...
            {
                tooltipText += Localization.instance.Localize("$jc_hold_advanced", $"<color=yellow><b>{Jewelcrafting.advancedTooltipKey.Value.MainKey}</b></color>");
            }
        }

        if (prophecySeed is not null)
        {
            if (ReplaceTooltipText.prophecyKeyDown)
            {
                int seed = int.Parse(prophecySeed);
                Random.State state = Random.state;
                Random.InitState(seed);

                ItemDrop.ItemData clone = item.Clone();
                ApplyCorruption(clone);
                container = clone.Data().Get<Sockets>()!;

                Random.state = state;
                tooltipText += "\n" + Localization.instance.Localize("$jc_prophecy_viewing");
            }
            else
            {
                tooltipText += "\n" + Localization.instance.Localize("$jc_prophecy_view", Localization.instance.Localize("<color=yellow><b>$KEY_Run</b></color>"));
            }
        }
        ...
        for (int i = 1; i <= Jewelcrafting.maxInternalNumberOfSockets; ++i)
        {
        ...
        {
            interact.GetComponent<TMP_Text>().text = tooltipText;
            interact.gameObject.SetActive(container?.boxSealed != true);
        }
    }
    ...
    private static class ReplaceTooltipText
    {
        ...
        public static bool prophecyKeyDown;

        private static void Postfix(UITooltip __instance)
        {
            if (__instance == UITooltip.m_current)
            {
                if (keyDown != Jewelcrafting.advancedTooltipKey.Value.IsPressed())
                {
                    keyDown = !keyDown;
                    UpdateSocketTooltip.Postfix(__instance);
                }

                if (prophecyKeyDown != ZInput.GetButton("Run"))
                {
                    prophecyKeyDown = !prophecyKeyDown;
                    UpdateSocketTooltip.Postfix(__instance);
                }
            }
        }
    ...
    private class DisplayEffectsOnGems
    {
        private static void Postfix(ItemDrop.ItemData item, ref string __result)
        {
            ...
            if (item.Data()["ProphecySeed"] is not null)
            {
                __result += "\n$jc_prophecy_flag";
            }
            else if (item.Data()["Corrupted Item"] is not null)
            {
                __result += "\n<color=red>$jc_corrupted_flag</color>";
            }
            else if (item.Data()["Blessed Item"] is not null)
            {
                __result += "\n<color=green>$jc_blessed_flag</color>";
            }
    ...
    private class CloseFakeSocketsContainer
    {
        ...
        public static void Prefix(InventoryGui __instance)
        {
            if (AddFakeSocketsContainer.openEquipment is not null && AddFakeSocketsContainer.openInventory is not null)
            {
                ...
                AddSocketIcons.containerCloseContainer.SetActive(false);
    ...
    public class OpenFakeSocketsContainer
    {
        public static bool Open(InventoryGui invGui, ItemDrop.ItemData? item)
        {
            ...
            if (AddFakeSocketsContainer.openInventory is { } inventory)
            {
                ...
                AddSocketIcons.containerCloseContainer.SetActive(true);
    ...
    private static void HandleSocketingFrameAndMirrors(ItemDrop.ItemData container, ItemDrop.ItemData item)
    {
        ...
        {
            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$jc_frame_max_sockets");
        }
        ...
        else if (container.m_shared.m_name == MiscSetup.divinityOrbName && item.Data()["Corrupted Item"] is not null)
        {
        ...
        else if (container.m_shared.m_name == MiscSetup.corruptionOrbName && item.Data()["Corrupted Item"] is not null && item.Data()["ProphecySeed"] is null)
        {
            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$jc_corrupted_recursion");
        }
        else if (container.m_shared.m_name == MiscSetup.prophecyOrbName && item.Data()["Corrupted Item"] is not null && item.Data()["ProphecySeed"] is null)
        {
            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$jc_corrupted_reroll_attempt");
        }
        ...
        {
            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "jc_orb_of_misfortune_already_blessed");
        }
        else if (container.m_shared.m_name == MiscSetup.corruptionOrbName && !Utils.IsSocketableItem(item))
        {
            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$jc_corrupted_not_socketable");
        }
        else if ((container.m_shared.m_name == MiscSetup.finalityOrbName || container.m_shared.m_name == MiscSetup.whimsicalityOrbName) && existingSockets?.socketedGems.Any(s => s.Corrupted) == true)
        {
            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$jc_corrupted_reroll_attempt");
        }
        else if ((container.m_shared.m_name == MiscSetup.finalityOrbName || container.m_shared.m_name == MiscSetup.whimsicalityOrbName) && existingSockets?.socketedGems.Any(s => s.Count > 0) != true)
        {
            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$jc_orb_of_finality_no_gem");
        }
        ...
        else if (!socketing && (!socketingFrame || existingSockets?.socketedGems.Count != Jewelcrafting.maximumNumberSockets.Value))
        {
            ...
            IEnumerator DelayedFrameDelete()
            {
                ...
                else if (container.m_shared.m_name == MiscSetup.finalityOrbName)
                {
                    GemLocation gemLocation = Utils.GetGemLocation(item.m_shared);
                    GemLocation itemLocation = Utils.GetItemGemLocation(item);

                    for (int i = 0; i < existingSockets!.socketedGems.Count; ++i)
                    {
                        if (existingSockets.socketedGems[i].Seed is { } bestSeeds)
                        {
                            float strength(Dictionary<string, uint>? seeds)
                            {
                                float sum = 0;
                                foreach (GemLocation location in new[] { gemLocation, itemLocation })
                                {
                                    if (Jewelcrafting.EffectPowers.TryGetValue(existingSockets.socketedGems[i].Name.GetStableHashCode(), out Dictionary<GemLocation, List<EffectPower>> locationPowers) && locationPowers.TryGetValue(location, out List<EffectPower> powers))
                                    {
                                        for (int j = 0; j < powers.Count; ++j)
                                        {
                                            FieldInfo[] minFields = powers[j].MinConfig.GetType().GetFields();
                                            FieldInfo[] maxFields = powers[j].MaxConfig.GetType().GetFields();
                                            for (int k = 0; k < minFields.Length; ++k)
                                            {
                                                if ((float)minFields[k].GetValue(powers[j].MinConfig) != (float)maxFields[k].GetValue(powers[j].MaxConfig))
                                                {
                                                    sum += Utils.CalcRealEffectPower(0, 1, k, powers[j].Type, powers[j].Effect, seeds);
                                                }
                                            }
                                        }
                                    }
                            ...
                            float bestValue = strength(bestSeeds);

                            // Track best values across rolls
                            for (int roll = 0; roll < Jewelcrafting.finalityOrbRerollAmount.Value; ++roll)
                            {
                                Dictionary<string, uint> seeds = bestSeeds.ToDictionary(kv => kv.Key, _ => Utils.GenerateSocketSeed());
                                float value = strength(seeds);
                            ...
                            existingSockets.socketedGems[i] = existingSockets.socketedGems[i] with { Seed = bestSeeds, Corrupted = true };
                        }
                    }

                    item.Data().Save();
                }
                else if (container.m_shared.m_name == MiscSetup.whimsicalityOrbName)
                {
                    for (int i = 0; i < existingSockets!.socketedGems.Count; ++i)
                    {
                        if (existingSockets.socketedGems[i].Seed is { } seeds)
                        {
                            existingSockets.socketedGems[i] = existingSockets.socketedGems[i] with { Seed = seeds.ToDictionary(kv => kv.Key, _ => Utils.GenerateSocketSeed()) };
                        }
                    }

                    item.Data().Save();
                }
                else if (container.m_shared.m_name == MiscSetup.corruptionOrbName)
                {
                    Random.State state = Random.state;
                    if (item.Data()["ProphecySeed"] is { } seedString)
                    {
                        int seed = int.Parse(seedString);
                        Random.InitState(seed);
                    }

                    ApplyCorruption(item);

                    Random.state = state;
                    item.Data().Remove("ProphecySeed");
                }
                else if (container.m_shared.m_name == MiscSetup.prophecyOrbName)
                {
                    item.Data().Save();
                    item.Data()["ProphecySeed"] = Random.Range(int.MinValue, int.MaxValue).ToString();
                    item.Data()["SocketsLock"] = "";
                    item.Data()["Corrupted Item"] = "";
                }
                else if (container.m_shared.m_name == MiscSetup.misfortuneOrbName)
                {
                    item.Data().Save();
                    item.Data()["Blessed Item"] = "";
                }
    ...
    public static void ApplyCorruption(ItemDrop.ItemData item)
    {
        Sockets? existingSockets = item.Data().Get<Sockets>();
        if (existingSockets is null)
        {
            existingSockets = item.Data().Add<Sockets>();
            existingSockets!.socketedGems.Clear();
        }

        List<Action> actions =
        [
            () => existingSockets.Item.m_quality = Math.Max(1, existingSockets.Item.m_quality + Random.Range(0, 2) * 2 - 1),
            () =>
            {
                if (Random.Range(0, 2) == 0)
                {
                    if (existingSockets.socketedGems.Count > 0)
                    {
                        existingSockets.socketedGems.RemoveAt(Random.Range(0, existingSockets.socketedGems.Count));
                    }
                }
                else
                {
                    SocketItem socket = new(socketableGemStoneNames[Random.Range(0, socketableGemStoneNames.Count)], corrupted: true);
                    socket.EnsureSeeds();
                    existingSockets.socketedGems.Add(socket);
                }
            },
            () =>
            {
                existingSockets.Item.m_quality = Random.Range(1, existingSockets.Item.m_shared.m_maxQuality + 2);
                existingSockets.socketedGems.Clear();
                for (int i = Random.Range(1, Jewelcrafting.maximumNumberSockets.Value + 2); i > 0; --i)
                {
                    SocketItem socket = new(socketableGemStoneNames[Random.Range(0, socketableGemStoneNames.Count)], corrupted: true);
                    socket.EnsureSeeds();
                    existingSockets.socketedGems.Add(socket);
            ...
            () =>
            {
                SocketItem socket = new("JC_Corrupted_Gem");
                socket.EnsureSeeds();
                existingSockets.socketedGems.Add(socket);
            },
        ];
        actions[Random.Range(0, actions.Count)]();
        if (existingSockets.socketedGems.Count == 0)
        {
            existingSockets.Info.Remove(existingSockets);
        }
        ...
        private static bool Prefix(InventoryGui __instance, ItemDrop.ItemData? item)
        {
            ...
            if (item.Data().Get<SocketBag>() is { } socketBag && Utils.ItemAllowedInGemBag(dragItem))
            {
                ...
                if (socketBag.socketedGems.FindIndex(s => s.Count == 0 || s.Name == "") is { } index and >= 0)
                {
                    ...
                    socketBag.socketedGems[index] = new SocketItem(dragItem.m_dropPrefab.name, count: dragItem.m_stack, seed: dragItem.Data().GetAll<SocketSeed>().ToDictionary(kv => kv.Key, kv => kv.Value.Seed), corrupted: dragItem.Data()["Corrupted Item"] is not null);
                    RemoveItem();
                }
